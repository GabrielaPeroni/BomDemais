...
class ShoppingRepository {
    ...
    suspend fun addItem(name: String, category: String = "") {
        val key = ref.push().key ?: return
        ref.child(key).setValue(ShoppingItem(id = key, name = name, category = category)).await()
    }
    ...
    suspend fun updateQuantity(item: ShoppingItem) = ref.child(item.id).child("quantityToBuy").setValue(item.quantityToBuy).await()
